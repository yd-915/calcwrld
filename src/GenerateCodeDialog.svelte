<script lang="ts">
  ...
  function codeTemplate(statement: CodeFunctionQueryStatement, result: Result | FiniteImagResult | MatrixResult) {
    ...
    """
    Function '${functionName}' automatically generated by CalcWrld
...
`
  ...
  $: if (statement && result && "generatedCode" in result && result.generatedCode) {
    try {
      ...
      generatedCode = `# Error generated code: ${e} If this error persists, report to support@calcwrld.space with the sheet that generates the error.`
    }
  } else {
    generatedCode = "";
  }
...
<div class="info">
  ...
  <div>
    ...
    <a href="mailto:support@calcwrld.space">support@calcwrld.space</a>
  </div>
</div>
